...
public class CreateTreeXml {
  ...
  private String target_orls;

  public CreateTreeXml(List<Map<String,String>> list) {
    this.listRoot = list;
  }

  public CreateTreeXml(List<Map<String,String>> list , String target_orl , String target_orls) {
    this.listRoot = list;
    this.target_orl = target_orl;
    this.target_orls = target_orls;
  }
  ...
  public Element xmlBulider() {
    ...
    for (int i = 0; i < total; i++ ) {
      ...
      while(iterator.hasNext()){
        String key = (String) iterator.next();
        String value = mapGet.get(key);
        if (StringUtils.equals(key,target_orl)) { category_orl = value; map_name = "m" + value; }
        mapItemPut.put(key,value);
      }
      ...
      mapItemPut.put(target_orls,category_orls);
