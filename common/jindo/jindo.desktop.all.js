...
 * NHN_Library:Jindo-2.7.0;JavaScript Framework;
...
jindo.$Jindo.checkVarType._throwException = function(aArgs, oRules, sFuncName) {
    ...
    if (/(\$\w+)#(\w+)?/.test(sFuncName)) {
        sURL = 'http://jindo.dev.naver.com/docs/jindo/archive/Jindo2-2.7.0/desktop/ko/classes/' + encodeURIComponent(RegExp.$1) + '.html' + "#method_"+RegExp.$2;
    }
 ...
 콜백코드는 넣었지만, 아직 SWF에서 응답헤더 지원 안함
	...
	abort : function(){
		...
		if(this._current_callback_id){
			...
			window["__"+jindoName+"2_callback"][this._current_callback_id] = function(data){
                window["__"+jindoName+"_callback"][this._current_callback_id](data);
            };
		}
	}
}).extend(jindo.$Ajax.RequestBase);
...
jindo.$Ajax.FrameRequest = klass({
	...
	send : function(data) {
		...
		window["__"+jindoName+"2_callback"][info.id] = function(id, data, header){
            window["__"+jindoName+"_callback"][info.id](id, data, header);
        };
...
/**
	...
	@example
		//함수도 할당할 수 있다.
		$Event.hook("textShadow",function(){
			if(isIE&&version>10){
				return "MsTextShadow";
			}else if(isSafari){
